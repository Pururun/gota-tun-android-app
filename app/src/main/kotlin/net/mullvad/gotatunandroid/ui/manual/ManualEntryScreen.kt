...
import net.mullvad.gotatunandroid.constant.Mtu
import net.mullvad.gotatunandroid.constant.Port
...
fun ManualEntryScreen(
  ...
  var address by remember {
    mutableStateOf(initialConfig?.interfaceConfig?.addresses?.joinToString(", ") ?: "")
  }
  var dns by remember {
    mutableStateOf(initialConfig?.interfaceConfig?.dns?.joinToString(", ") ?: "")
  }
  ...
  fun validate(): Boolean {
    ...
    mtuError =
        mtu.isNotBlank() && (mtu.toIntOrNull()?.let { it !in Mtu.MIN_MTU..Mtu.MAX_MTU } ?: true)
    endpointPortError =
        endpointPort.isNotBlank() &&
            (endpointPort.toIntOrNull()?.let { it !in Port.MIN_PORT..Port.MAX_PORT } ?: true)
