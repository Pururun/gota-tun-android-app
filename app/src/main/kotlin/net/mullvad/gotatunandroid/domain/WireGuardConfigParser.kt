            ...
            "interface" ->
                when (key.lowercase()) {
                  ...
                  "mtu" -> {
                    ...
                    if (parsed != null && parsed in Mtu.MIN_MTU..Mtu.MAX_MTU) mtu = parsed
                  }
                }
            "peer" ->
                when (key.lowercase()) {
                  ...
                  "endpoint" -> {
                    ...
                    if (lastColon > 0) {
                      ...
                      peerEndpointPort =
                          if (port != null && port in Port.MIN_PORT..Port.MAX_PORT) port else null
                    } else {
  ...
  fun serialize(config: VpnConfig): String = buildString {
    ...
    config.peers.forEach { peer ->
      ...
      if (peer.endpointHost.isNotEmpty()) {
        val port = peer.endpointPort ?: (0..65535).random()
